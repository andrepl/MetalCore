...
import com.norcode.bukkit.metalcore.chat.Text;
...
public class BaseCommand implements TabExecutor {
	...
	public BaseCommand(MetalCorePlugin plugin, String name, String[] aliases, String requiredPermission, String[] help) {
		this.plugin = plugin;
		this.name = name;
	...
	protected void showHelp(CommandSender sender, String label, LinkedList<String> args) {
		...
		if (subcommands.size() > 0) {
			new Text("Subcommands: " + StringUtils.join(filterByPermission(sender, subcommands).keySet(), ", ")).sendTo(sender);
		}
	}
